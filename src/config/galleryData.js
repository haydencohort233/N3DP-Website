// src/config/galleryData.js
const galleryData = [
  {
    id: "mickey-kcup-holder",
    title: "Mickey Mouse K-Cup Holder",
    description: "A Mickey-themed Ghost that holds your Keurig Coffee Cup!",
    date: "2026-01-12",
    category: "Kitchen",
    tags: ["Custom", "Disney", "Mickey", "K-Cup", "Coffee", "Halloween"],
    src: "/assets/gallery/photo1.png",
    photos: [
      {
        src: "/assets/gallery/photo1.png",
        label: "Black",
        tags: ["Black"],
      },
      {
        src: "/assets/gallery/photo1-1.png",
        label: "Red",
        tags: ["Red", "Bow", "Minnie"],
        title: "Minnie Mouse Keurig Coffee Cup Holder",
        description:
          "A Minnie Mouse Keurig Coffee Holder, featuring a cute bow and her famous polka dots.",
      },
    ],
  },

  {
    id: "halloween-bookmarks",
    legacyId: 2,
    title: "Halloween Bookmarks",
    description: "Adorable bookmarks for reading! These come in so many styles - or request something similar!",
    date: "2024-02-05",
    category: "Halloween",
    tags: ["Halloween", "Books", "Custom"],
    src: "/assets/gallery/photo2.png",
    photos: [
      { src: "/assets/gallery/photo2.png" },
    ],
  },

  {
    id: "smoker-stash-box",
    legacyId: 3,
    title: "Smoker Stash Box",
    description:
      "An all-in-one box for smokers! Has a space for a grinder, 3 lighters, dab pens, wipes, and Q-tip holders!",
    date: "2024-02-05",
    category: "Miscellaneous",
    tags: ["Custom", "Smoker", "Weed"],
    src: "/assets/gallery/photo3.png",
    photos: [{ src: "/assets/gallery/photo3.png" }],
  },

  {
    id: "christmas-ornaments",
    legacyId: 4,
    title: "Personalized Christmas Ornaments",
    description: "Beautiful Christmas Ornaments that can be customized to anyone's name to be more personal",
    date: "2024-02-05",
    category: "Christmas",
    tags: ["Christmas", "Ornament", "Gifts"],
    src: "/assets/gallery/photo4.png",
    photos: [{ src: "/assets/gallery/photo4.png" }],
  },

  {
    id: "mechanical-gear",
    legacyId: 5,
    title: "Mechanical Gear",
    description:
      "A replacement gear for a discontinued part from Germany. Requested by a Museum to repair their carousel.",
    date: "2023-02-05",
    category: "Mechanical",
    tags: ["Custom", "Gears", "Resin"],
    src: "/assets/gallery/photo5.png",
    photos: [{ src: "/assets/gallery/photo5.png" }],
    quotable: false
  },

  {
    id: "money-maze",
    legacyId: 6,
    title: "Money Maze",
    description:
      "A puzzle with a gift inside! Solve the maze by twisting and turning. Once finished it comes apart revealing what's inside.",
    date: "2023-12-23",
    category: "Christmas",
    tags: ["Christmas", "Gifts", "Puzzle"],
    src: "/assets/gallery/photo6.png",
    photos: [{ src: "/assets/gallery/photo6.png" }],
  },

  {
    id: "zelda-master-sword",
    legacyId: 7,
    title: "Zelda Master Sword",
    description: "A sword prop from The Legend of Zelda.",
    date: "2022-08-15",
    category: "Video Games",
    tags: ["Zelda", "Sword", "Props", "Video Games"],
    src: "/assets/gallery/photo7.png",
    photos: [{ src: "/assets/gallery/photo7.png" }],
    quotable: false
  },

  {
    id: "hello-kitty-bead-tray",
    legacyId: 8,
    title: "Hello Kitty Bead Tray",
    description: "A custom bead tray in the shape of Hello Kitty.",
    date: "2024-03-11",
    category: "Arts & Crafts",
    tags: ["Custom", "Crafts", "Hello Kitty", "Multicolor"],
    src: "/assets/gallery/photo8.png",
    photos: [{ src: "/assets/gallery/photo8.png" }],
  },

  {
    id: "glow-figurines",
    legacyId: 9,
    title: "Glow-in-the-Dark Figurines",
    description:
      "100 figurines made in glow-in-the-dark resin using a resin printer. Requested by CakeHoarder for their Twitch community.",
    date: "2023-06-05",
    category: "Figurines",
    tags: ["Custom", "Figurines", "Commission", "Resin", "Glow"],
    src: "/assets/gallery/photo9.png",
    photos: [{ src: "/assets/gallery/photo9.png" }],
  },

  {
    id: "candle-wick-holders",
    legacyId: 10,
    title: "Candle Wick Holders",
    description: "Used for pouring candle wax and making candles, holds your candle wick in place while pouring to have a clean candle.",
    date: "2024-02-05",
    category: "Businesses",
    tags: ["Custom", "Candles", "Request"],
    src: "/assets/gallery/photo10.png",
    photos: [{ src: "/assets/gallery/photo10.png" }],
  },

  {
    id: "heart-ring-bowl",
    legacyId: 11,
    title: "Heart Shaped Ring Bowl",
    description:
      "A heart-shaped bowl made for holding rings or jewelry. Great gift idea.",
    date: "2024-02-12",
    category: "Valentine's Day",
    tags: ["Gifts", "Valentine's Day", "Heart", "Love"],
    src: "/assets/gallery/photo11.png",
    photos: [{ src: "/assets/gallery/photo11.png" }],
  },

  {
    id: "easter-fidgets",
    legacyId: 12,
    title: "Easter Fidget Toys",
    description:
      "Flexible fidget toys for Easter (bunny + peep). Fun to flex and wiggle around.",
    date: "2022-04-19",
    category: "Easter",
    tags: ["Easter", "Gifts", "Fidget"],
    src: "/assets/gallery/photo12.png",
    photos: [{ src: "/assets/gallery/photo12.png" }],
  },

  {
    id: "doodlebob-pencil-holder",
    legacyId: 13,
    title: "DoodleBob Pencil Holder",
    description:
      "A DoodleBob-themed pencil holder. Holds pens, sharpies, or anything similar.",
    date: "2024-02-05",
    category: "Gifts",
    tags: ["Gifts", "SpongeBob", "Figurines", "Cartoons"],
    src: "/assets/gallery/photo13.png",
    photos: [{ src: "/assets/gallery/photo13.png" }],
  },

  {
    id: "sidewalk-miniatures",
    legacyId: 14,
    title: "Sidewalk Miniatures",
    description: "Miniature sidewalk sections created for tabletop displays, miniature towns, and custom diorama projects.",
    date: "2024-02-05",
    category: "Miniatures",
    tags: ["Miniatures", "Diorama", "Tabletop", "Custom"],
    src: "/assets/gallery/photo14.png",
    photos: [{ src: "/assets/gallery/photo14.png" }],
  },

  {
    id: "coffin-kcup-holder",
    legacyId: 15,
    title: "Blood Oozing Coffin Keurig Cup Holder",
    description:
      "A coffin-themed K-Cup holder—perfect for spooky season coffee setups.",
    date: "2025-10-30",
    category: "Halloween",
    tags: ["Custom", "Halloween", "Keurig", "Coffee", "Black"],
    src: "/assets/gallery/photo15.png",
    photos: [
      {
        src: "/assets/gallery/photo15.png",
        date: "2023-02-05",
      },
    ],
  },

  {
    id: "ligogram-logo",
    legacyId: 16,
    title: "Ligogram Company Logo",
    description: "A ligogram lets lights through to show an image. This can be done with family photos, company logos and so much more.",
    date: "2024-02-05",
    category: "Businesses",
    tags: ["Custom", "Ligogram", "Lightbox", "Logo"],
    src: "/assets/gallery/photo16.png",
    photos: [{ src: "/assets/gallery/photo16.png" }],
  },

  {
    id: "ocarina",
    legacyId: 17,
    title: "Ocarina",
    description:
      "A blue ocarina based on Legend of Zelda: Ocarina of Time. Purely decorational.",
    date: "2024-02-05",
    category: "Video Games",
    tags: ["Music", "Instrument", "Zelda", "Ocarina"],
    src: "/assets/gallery/photo17.png",
    photos: [{ src: "/assets/gallery/photo17.png" }],
  },

  {
    id: "miniature-forklift",
    legacyId: 18,
    title: "Miniature Yellow Forklift",
    description: "A forklift for your desk or shelves, it even includes a pallet for your sticky notes.",
    date: "2024-02-05",
    category: "Gifts",
    tags: ["Desktop", "Father's Day", "Gifts"],
    src: "/assets/gallery/photo18.png",
    photos: [
      {
        src: "/assets/gallery/photo18.png", // Yellow Forklift
        tags: ["Yellow"],
      },
      {
        src: "/assets/gallery/photo18-1.png", // Blue Forklift
        title: "Miniature Blue Forklift",
        tags: ["Blue"],
        date: "2026-01-01",
      },
    ],
  },

  {
    id: "business-card-holders",
    legacyId: 19,
    title: "Business Card Holders",
    description:
      "Custom business card holders styled in the shape of a company logo.",
    date: "2024-02-05",
    category: "Businesses",
    tags: ["Custom", "Business Card"],
    src: "/assets/gallery/photo19.png",
    photos: [{ src: "/assets/gallery/photo19.png" }],
  },

  {
    id: "cat-succulent-pot",
    legacyId: 20,
    title: "Cat Head Succulent Pot",
    description:
      "A succulent pot for cat lovers with a drain hole on the bottom.",
    date: "2023-07-05",
    category: "Gifts",
    tags: ["Cat", "Planter", "Succulent", "Home Decor"],
    src: "/assets/gallery/photo20.png",
    photos: [{ src: "/assets/gallery/photo20.png" }],
  },

  {
    id: "giant-lego-skeleton",
    legacyId: 21,
    title: "Giant LEGO Skeleton",
    description: "Giant LEGO Skeleton! It's 1 foot tall and has posable limbs. Get this guy to decorate your house!",
    date: "2024-02-05",
    category: "Gifts",
    tags: ["Lego", "Halloween", "Skeleton"],
    src: "/assets/gallery/photo21.png",
    photos: [{ src: "/assets/gallery/photo21.png" }],
  },

  {
    id: "stanley-chapstick-holder",
    legacyId: 22,
    title: "Red Stanley Cup Chapstick Holder",
    description:
      "A mount to hold chapstick on the handle of your Stanley cup. Key holder option available.",
    date: "2024-02-05",
    category: "Gifts",
    tags: ["Custom", "Stanley Cup", "Accessory"],
    src: "/assets/gallery/photo22.png",
    photos: [
      {
        title: "Red Stanley Cup Chapstick Holder",
        src: "/assets/gallery/photo22.png", // Red
        tags: ["Red"],
        date: "2023-02-05",
      },
      {
        title: "White Stanley Cup Chapstick Holder",
        src: "/assets/gallery/photo22-1.png", // White
        tags: ["White"],
        date: "2023-02-05",
      },
      {
        title: "Pink Stanley Cup Chapstick Holder",
        src: "/assets/gallery/photo22-2.png", // Pink
        tags: ["Pink"],
        date: "2023-02-05",
      },
      {
        title: "Black Stanley Cup Chapstick Holder",
        src: "/assets/gallery/photo22-3.png", // Black
        tags: ["Black"],
        date: "2023-02-05",
      },
    ],
  },

  {
    id: "heart-ash-tray",
    legacyId: 23,
    title: "Heart-Shaped Ash Tray",
    description:
      "An ash tray with a poker—great for cleaning bowls and keeping things tidy.",
    date: "2024-02-05",
    category: "Valentine's Day",
    tags: ["Custom", "Gifts", "Valentine's Day", "Heart", "Pink"],
    src: "/assets/gallery/photo23.png",
    photos: [
      {
        src: "/assets/gallery/photo23.png", // Tray Only
        tags: ["Ash Tray"],
        date: "2023-02-05",
      },
      {
        src: "/assets/gallery/photo23-1.png", // Valentine's Bowl Set
        title: "Heart Ash Tray with Bowl Holder & Weed Shovel",
        date: "2023-02-05",
        description: "The complete Heart Ash Tray Colleciton - Tray, Shovel, Bowl Holder",
      },
      {
        src: "/assets/gallery/photo23-2.png", // Valentine's Shovel
        title: "Heart-Shaped Weed Shovel",
        tags: ["Shovel"],
        date: "2023-02-05",
        description: "A heart-shaped weed shovel, can also be a keychain or hang from a lanyard.",
      },
      {
        src: "/assets/gallery/photo23-3.png", // Valentine's Bowl Holder
        title: "Heart-Shaped Bong Bowl Holder",
        tags: ["Weed"],
        date: "2023-02-05",
        description: "Simple -- It holds your bowl while you pack it for your next smoke!",
      },
    ],
  },

  {
    id: "miniature-sidewalks",
    legacyId: 24,
    title: "Miniature Sidewalks",
    description: "A set of miniature sidewalks for a miniature town project.",
    date: "2024-02-05",
    category: "Miniatures",
    tags: ["Miniatures", "Gifts"],
    src: "/assets/gallery/photo24.png",
    photos: [{ src: "/assets/gallery/photo24.png" }],
  },

  {
    id: "wind-conduit-mounts",
    legacyId: 25,
    title: "Wind Conduit Mounts",
    description: "Custom conduit mounting brackets designed for agricultural wind systems and outdoor installations.",
    date: "2024-02-05",
    category: "Businesses",
    tags: ["Custom", "Request"],
    src: "/assets/gallery/photo25.png",
    photos: [{ src: "/assets/gallery/photo25.png" }],
  },

  {
    id: "miniature-movie-theater",
    legacyId: 26,
    title: "Miniature Movie Theater",
    description: "A miniature movie theater requested by a customer.",
    date: "2024-02-05",
    category: "Miniatures",
    tags: ["Commission"],
    src: "/assets/gallery/photo26.png",
    photos: [{ src: "/assets/gallery/photo26.png" }],
    quotable: false
  },

  {
    id: "lighter-sleeves",
    legacyId: 27,
    title: "Lighter Sleeves",
    description: "A sleeve for your lighter to customize it further or attach to a lanyard/keychain.",
    date: "2024-01-16",
    category: "Miscellaneous",
    tags: ["Gifts", "Smoker", "Sleeve"],
    src: "/assets/gallery/photo27.png",
    photos: [{ src: "/assets/gallery/photo27.png" }],
  },

  {
    id: "real-estate-card-holder",
    legacyId: 28,
    title: "Real Estate Business Card Holder",
    description: "A custom real estate business card holder shaped like a miniature yard sign. Perfect for Realtor desks, offices, and open houses.",
    category: "Businesses",
    tags: ["Real Estate", "Business", "Business Cards", "Office"],
    src: "/assets/gallery/photo28.png",
    photos: [{ src: "/assets/gallery/photo28.png" }],
  },

  {
    id: "business-logo-maximum-sports",
    legacyId: 29,
    title: "3D Business Logo",
    description: "Get a custom 3D printed business logos designed for your office, trade shows, storefront displays, or promotional events.",
    date: "2023-6-08",
    category: "Businesses",
    tags: ["Business", "Logo", "Signage", "Custom"],
    src: "/assets/gallery/photo29.png",
    photos: [{ src: "/assets/gallery/photo29.png", label: "Business 3D Logo" }],
  },

  {
    id: "winged-skull-hat",
    legacyId: 30,
    title: "Skull with Wings and Hat",
    description:
      "A giant scary skull with a pair of wings and a cool hat. Customer request.",
    date: "2025-12-15",
    category: "Miscellaneous",
    tags: ["Custom", "Request", "Skull"],
    src: "/assets/gallery/photo30.png",
    photos: [{ src: "/assets/gallery/photo30.png", label: "Winged Skull" }],
  },

  {
    id: "heart-shape-tealight-candle-holders",
    legacyId: 31,
    title: "Heart-Shaped Tealight Candle Holders",
    description: "Cute tealight holders shaped like hearts, perfect for a romantic evening, or a Valentine's Day surprise.",
    date: "2025-12-15",
    category: "Valentine's Day",
    tags: ["Heart", "Valentine's Day", "Candle", "Cute"],
    src: "/assets/gallery/photo31.png",
    photos: [
      {
        src: "/assets/gallery/photo31.png",
        title: "Red Heart Tealight Holder",
        label: "Red",
        tags: ["Red"],
      },
      {
        src: "/assets/gallery/photo31-1.png",
        label: "Purple",
        tags: ["Purple"],
        title: "Purple Heart Tealight Holder",
        description: "A purple heart-shaped tealight holder designed to add warmth and charm to any room.",
      },
      ],  
    },

    {
    id: "mini-ice-cooler-soda",
    legacyId: 32,
    title: "Miniature Ice Coolers",
    description:
      "These are miniature ice coolers perfect to hold a single can of soda. So cute!",
    date: "2025-12-15",
    category: "Miscellaneous",
    tags: ["Miniature", "Cooler", "Soda", "Desk Decor"],
    src: "/assets/gallery/photo32.png",
    photos: [{ src: "/assets/gallery/photo32.png" }],
  },

  {
    id: "stranger-things-demogorgon-statue-bust",
    legacyId: 33,
    title: "Stranger Things Demogorgon Bust",
    description:
      "An awesome bust of the Demogorgon from Stranger Things.",
    date: "2023-10-15",
    category: "Miscellaneous",
    tags: ["Stranger Things", "Demogorgon", "Horror", "Display Piece"],
    src: "/assets/gallery/photo33.png",
    photos: [{ src: "/assets/gallery/photo33.png" }],
  },

  {
    id: "kcup-coffee-holder-christmas-tree",
    legacyId: 34,
    title: "Christmas Tree Keurig Cup Holder",
    description:
      "A Christmas themed K-Cup Holder as a Christmas tree with a Garland, part of our K-Cup Collection",
    date: "2025-12-20",
    category: "Christmas",
    tags: ["Gifts", "Christmas", "K-Cup Holder", "Coffee", "Kitchen"],
    src: "/assets/gallery/photo34.png",
    photos: [{ src: "/assets/gallery/photo34.png" }],
  },

  {
    id: "designer-straw-toppers",
    legacyId: 35,
    title: "Chanel Straw Toppers",
    description:
      "Designer logo shaped Straw Toppers. We have Coach, Louis Vuitton, Fendi, Gucci and any others on request.",
    date: "2024-4-10",
    category: "Kitchen",
    tags: ["Straw Topper", "Cup Accessory", "Designer Inspired", "Custom"],
    src: "/assets/gallery/photo35.png",
    photos: [
      { 
        src: "/assets/gallery/photo35.png", 
        title: "Chanel Straw Toppers",
        label: "Chanel Straw Toppers",
        tags: ["Chanel"]
      },
      { 
        src: "/assets/gallery/photo35-1.png", 
        title: "Louis Vuitton Straw Toppers",
        label: "Louis Vuitton Straw Toppers",
        tags: ["Louis Vuitton"] 
      },
      { 
        src: "/assets/gallery/photo35-2.png", 
        title: "Fendi Straw Toppers",
        label: "Fendi Straw Toppers",
        tags: ["Fendi"] 
      }
    ],
  },

    {
    id: "skull-guitar-pick-holder",
    legacyId: 36,
    title: "Skull Guitar Pick Holder",
    description:
      "A fun small skull to hold several guitar picks to look like he has mohawk!",
    date: "2024-11-20",
    category: "Gifts",
    tags: ["Gifts", "Music", "Guitar"],
    src: "/assets/gallery/photo36.png",
    photos: [{ src: "/assets/gallery/photo36.png" }],
  },

  {
    id: "pumpkin-minnie-straw-topper",
    legacyId: 37,
    title: "Pumpkin Minnie Straw Toppers",
    description:
      "Straw Topper shaped like a Pumpkin Minnie with colored bow, or a Mickey Mouse version.",
    date: "2024-10-21",
    category: "Kitchen",
    tags: ["Disney", "Minnie Mouse", "Halloween", "Straw Topper"],
    src: "/assets/gallery/photo37.png",
    photos: [{ src: "/assets/gallery/photo37.png" }],
  },

  {
    id: "candy-corn-kcup-holder",
    legacyId: 38,
    title: "Candy Corn K-Cup Holder",
    description:
      "A keurig holder in the shape of a Candy Corn. Part of our Keurig Holder Collection for every month of the year.",
    date: "2024-10-25",
    category: "Halloween",
    tags: ["Gifts", "Halloween", "K-Cup", "Coffee", "Keurig"],
    src: "/assets/gallery/photo38.png",
    photos: [{ src: "/assets/gallery/photo38.png" }],
  },

  {
    id: "donkey-kong-barrel-cupholder",
    legacyId: 39,
    title: "Donkey Kong Pencil Holder",
    description:
      "An awesome barrel from Donkey Kong, this will hold all your pens and pencils!",
    date: "2026-2-1",
    category: "Nintendo",
    tags: ["Gifts", "Donkey Kong", "Mario", "Desktop"],
    src: "/assets/gallery/photo39.png",
    photos: [{ src: "/assets/gallery/photo39.png" }],
  },

  {
    id: "hearts-kcup-holder",
    legacyId: 40,
    title: "Double Hearts K-Cup Holder",
    description:
      "Another Keurig Pod Holder, this theme is for Valentine's Day or anything love. It holds up to 9 cups.",
    date: "2026-2-3",
    category: "Valentine's Day",
    tags: ["Gifts", "Valentine's Day", "Love", "Keurig", "Coffee", "Kitchen"],
    src: "/assets/gallery/photo40.png",
    photos: [{ src: "/assets/gallery/photo40.png" }],
  },

  {
    id: "innout-sauce-holders",
    legacyId: 41,
    title: "In-N-Out Sauce Holders",
    description:
      "A set of In-N-Out sauce holders (for their sauce paper cups) designed to go around your straw. Slide it down and insert your paper cups and get dipping!",
    date: "2026-2-8",
    category: "Kitchen",
    tags: ["Food", "Sauce Holder", "Fast Food", "Cup Accessory"],
    src: "/assets/gallery/photo41.jpg",
    photos: [{ src: "/assets/gallery/photo41.jpg" }],
  },

  {
    id: "lego-brick-flower-vase",
    legacyId: 42,
    title: "Lego Brick Flower Vase",
    description:
      "A 2x4 LEGO Brick for a vase! Now that's awesome. Put your real or fake flowers in here as a stunning decoration.",
    date: "2026-2-11",
    category: "Home Decor",
    tags: ["LEGO", "Flower Vase", "Home Decor", "Flowers"],
    src: "/assets/gallery/photo42.png",
    photos: [{ src: "/assets/gallery/photo42.png" }],
  },

  {
    id: "mario-airpods-case",
    legacyId: 43,
    title: "Mario AirPods Mystery Block Case",
    description:
      "A mystery block case for your AirPods Pro - No glue or screws required. Perfect for any Mario fan.",
    date: "2026-3-5",
    category: "Gifts",
    tags: ["Mario", "Nintendo"],
    src: "/assets/gallery/photo43.png",
    photos: [{ src: "/assets/gallery/photo43.png" }],
  },

  {
    id: "mario-mushroom-piggybank",
    legacyId: 44,
    title: "Mario Piggy Bank",
    description:
      "A Mario Mushroom that works as a piggy bank, the head unscrews off when it's full! These come small or large.",
    date: "2026-3-10",
    category: "Nintendo",
    tags: ["Gifts", "Nintendo", "Mushroom", "Mario"],
    src: "/assets/gallery/photo44.png",
    photos: [{ src: "/assets/gallery/photo44.png" }],
  },

  {
    id: "easter-peeps-keychain",
    legacyId: 45,
    title: "Keychain Peeps",
    description: "Cute Peeps-inspired Easter keychains available in multiple colors. Perfect for baskets, gifts, and spring celebrations.",
    date: "2026-3-20",
    category: "Easter",
    tags: ["Easter", "Peeps", "Keychain", "Spring"],
    src: "/assets/gallery/photo45.png",
    photos: [{ src: "/assets/gallery/photo45.png" }],
  },
  
  {
    id: "dragon-dagger-prop",
    legacyId: 46,
    title: "RuneScape Dragon Dagger (p++)",
    description: "A detailed replica of the iconic Dragon Dagger (p++) from RuneScape. Great for cosplay, display collections, and gaming rooms.",
    date: "2026-3-30",
    category: "Video Games",
    tags: ["RuneScape", "Dragon Dagger", "Prop", "Video Games"],
    src: "/assets/gallery/photo46.png",
    photos: [{ src: "/assets/gallery/photo46.png" }],
  },
    
  {
    id: "gag-screwdrivers",
    legacyId: 47,
    title: "Prank Double Screwdrivers",
    description: "A novelty prank screwdriver featuring two heads instead of one. A fun desk accessory and conversation starter.",
    date: "2026-4-01",
    category: "Props",
    tags: ["Prank", "Tool", "Novelty", "Desk Decor"],
    src: "/assets/gallery/photo47.png",
    photos: [{ src: "/assets/gallery/photo47.png" }],
  },
      
  {
    id: "bobomb-container",
    legacyId: 48,
    title: "Mario Bob-Omb Container",
    description:
      "An adorable Bob-Omb to sit on your shelf. Take off the cap and store anything you want inside, maybe jelly beans?",
    date: "2026-4-10",
    category: "Nintendo",
    tags: ["Mario", "Nintendo", "Video Games", "Container", "Display Piece"],
    src: "/assets/gallery/photo48.png",
    photos: [{ src: "/assets/gallery/photo48.png" }],
  },
        
  {
    id: "gamecube-controller-stand",
    legacyId: 49,
    title: "GameCube Controller Stand",
    description:
      "Used to safely display your controllers. Very lightweight and simple design.",
    date: "2026-4-15",
    category: "Nintendo",
    tags: ["GameCube", "Controller Stand", "Nintendo", "Gaming Setup"],
    src: "/assets/gallery/photo49.png",
    photos: [{ src: "/assets/gallery/photo49.png" }],
  },
        
  {
    id: "gamecube-game-stand",
    legacyId: 50,
    title: "GameCube Game Stand (Japanese)",
    description:
      "A custom GameCube Game Holder for Japanese GameCube games. Regular game holder can also be purchased upon request.",
    date: "2026-4-22",
    category: "Nintendo",
    tags: ["GameCube", "Game Display", "Nintendo", "Collector"],
    src: "/assets/gallery/photo50.png",
    photos: [{ src: "/assets/gallery/photo50.png" }],
  },
          
  {
    id: "blockbuster-vhs-stand",
    legacyId: 51,
    title: "Blockbuster VHS Display Stand",
    description: "A retro-inspired display stand for showcasing VHS tapes, DVDs, Blu-rays, and favorite movie collectibles.",
    date: "2026-4-30",
    category: "Home Decor",
    tags: ["Blockbuster", "VHS", "Retro", "Movie Collection"],
    src: "/assets/gallery/photo51.png",
    photos: [{ src: "/assets/gallery/photo51.png" }],
  },
            
  {
    id: "oreo-stash-container",
    legacyId: 52,
    title: "Oreo Stash Container",
    description: "Lifelike Oreo Cookie that you unscrew to put things inside of. Order it even bigger or exact Oreo size!",
    date: "2026-5-10",
    category: "Gifts",
    tags: ["Food", "Oreo", "Gifts"],
    src: "/assets/gallery/photo52.png",
    photos: [{ src: "/assets/gallery/photo52.png" }],
  },
            
  {
    id: "soda-can-koozie",
    legacyId: 53,
    title: "Soda Can Koozie",
    description: "A cute Igloo Container designed to hold your 12oz soda cans! Keeps it cooler longer and you can put the lid on!",
    date: "2026-5-15",
    category: "Gifts",
    tags: ["Household", "Summer", "Gifts"],
    src: "/assets/gallery/photo53.png",
    photos: [{ src: "/assets/gallery/photo53.png" }],
  },
            
  {
    id: "little-tikes-sandbox",
    legacyId: 54,
    title: "Little Tike's Mini Sandbox",
    description:
      "Have your own piece of nostalgia at home with a Tike's Sandbox! Great for your desk or a Zen garden at home.",
    date: "2026-5-20",
    category: "Gifts",
    tags: ["Retro", "Household", "Gifts"],
    src: "/assets/gallery/photo54.png",
    photos: [{ src: "/assets/gallery/photo54.png" }],
  },
            
  {
    id: "n64-game-display",
    legacyId: 55,
    title: "Nintendo 64 Game Display",
    description: "A Nintendo 64 display tower designed to organize and showcase your favorite N64 game cartridges.",
    date: "2026-5-25",
    category: "Nintendo",
    tags: ["Retro", "Gifts", "Video Games"],
    src: "/assets/gallery/photo55.png",
    photos: [{ src: "/assets/gallery/photo55.png" }],
  },
];

export default galleryData;
